...
// CmdSLALetterCobra returns the slaletter cobra command for integration into govex CLI.
func CmdSLALetterCobra() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "slaletter",
		Short: "Generate SLA exception notification letters",
		Long: `slaletter generates SLA exception notification letters for security findings.
...
func cmdSchema() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "schema",
		Short: "Output JSON Schema for the SLA exception format",
		Long: `Output the JSON Schema that describes the SLA exception JSON format.
...
func cmdExample() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Output an example SLA exception JSON file that can be used as a template.
...
func cmdGenerate() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "generate",
		Short: "Generate SLA exception letter",
		Long: `Generate an SLA exception letter from JSON input or command-line parameters.
  ...
  govex slaletter generate \
    ...
    --output-md letter.md`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if inputFile != "" {
				exc, err = ReadFile(inputFile)
				if err != nil {
					return fmt.Errorf("failed to read input file: %w", err)
				}
			} else {
				exc, err = buildExceptionFromFlags(
				...
				if err != nil {
					return fmt.Errorf("failed to build exception from flags: %w", err)
				}
			}
			...
			if outputJSON != "" {
				if err := exc.WriteFile(outputJSON); err != nil {
			...
			if outputMD != "" {
				if err := exc.WriteMarkdownFile(outputMD); err != nil {
	...
	// SLA
	cmd.Flags().IntVar(&slaTargetDays, "sla-target-days", 0, "SLA target days for remediation")
	cmd.Flags().StringVar(&slaOriginalDate, "sla-original-date", "", "Original SLA due date (YYYY-MM-DD)")
	cmd.Flags().StringVar(&slaNewDate, "sla-new-date", "", "New SLA due date (YYYY-MM-DD)")

	// Delay/Risk/Mitigation
	cmd.Flags().StringArrayVar(&delayReasons, "delay-reason", nil, "Delay reason (can be specified multiple times)")
	...
	// Approver (optional)
	cmd.Flags().StringVar(&approverName, "approver-name", "", "Exception approver name")
	cmd.Flags().StringVar(&approverTitle, "approver-title", "", "Exception approver title")
	cmd.Flags().StringVar(&approverDate, "approver-date", "", "Exception approval date (YYYY-MM-DD)")
...
func buildExceptionFromFlags(
	...
	if len(milestones) > 0 {
		for _, m := range milestones {
			...
			exc.Milestones = append(exc.Milestones, Milestone{
				Phase:          phase,
				Description:    parts[1],
				TargetDate:     parts[2],
	...
	if len(escalationPolicy) > 0 {
		exc.EscalationPolicy = escalationPolicy
	}

	exc.SetSubjectFromFinding()
